...
//  PointingBrowser
//
//  Created by yangjian on 2023/3/29.
//
...
extension GADUtil {
    ...
    func show(_ position: ADPosition, from vc: UIViewController? = nil , completion: @escaping (ADBaseModel?)->Void) {
        ...
        case .interstitial:
            /// 有廣告
            if let ad = loadAD?.loadedArray.first as? InterstitialADModel, !AppUtil.shared.enterbackground, !isADLimited {
        ...
        case .native:
            if let ad = loadAD?.loadedArray.first as? NativeADModel, !AppUtil.shared.enterbackground, !isADLimited {
                /// 预加载回来数据 当时已经有显示数据了
...
extension InterstitialADModel {
    ...
    override func dismiss() {
        if let vc = AppUtil.shared.appdelegate?.window?.rootViewController?.presentedViewController {
